...
static int run_algorithm(const char *machine, const char *algorithm,
    ...
    if (strcmp(algorithm, "avx2") == 0) {
        ...
        impl[0] = (struct implementation){"ours-9x", ChaCha20_ctr32};
        ...
        const char *names[] = {"ours-17x", "openssl-avx512", "intel-avx512"};
        for (size_t i = 0; i < sizeof(sizes) / sizeof(sizes[0]); i++)
            for (size_t j = 0; j < sizeof(names) / sizeof(names[0]); j++)
                bench_print_unsupported(algorithm, machine, sizes[i], names[j]);
        return 0;
    } else {
        impl[0] = (struct implementation){"ours-17x", ChaCha20_ctr32};
